...
function listingo_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'listingo' ),
        'next_string'     => esc_html__( '&raquo;', 'listingo' ),
        'before_output'   => '<div class="listingo-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'listingo_pagination_defaults', $defaults )
    );
