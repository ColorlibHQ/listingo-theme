...
// Rating function
function listingo_rating_function( $rating = 5 ) {
	$html = '';
	for( $i = 0; $i < $rating; $i++ ) {
...
// Tags function
function listingo_get_tax_function( $taxonomy = 'listingo-tag' ) {
...
// Tags function
function listingo_get_taxonomies( $taxonomy = 'listing_category', $hide_empty = false ) {
	$args = [
		'taxonomy' => $taxonomy,
...
function listingo_meta( $id = '', $value_type = true ){
    
    $value = get_post_meta( get_the_ID(), '_listingo_'.$id, $value_type );
    
    return $value;
}

// listingo
function listingo_get_styles_from_meta( $footer_bg_opt ) {
	$bg_styles = '';
...
// Blog Date Permalink
function listingo_blog_date_permalink(){
...
// Blog Excerpt Length
if ( ! function_exists( 'listingo_excerpt_length' ) ) {
	function listingo_excerpt_length( $limit = 30 ) {
...
if ( ! function_exists( 'listingo_posted_comments' ) ) :
    function listingo_posted_comments(){
        
        $comments_num = get_comments_number();
        if( comments_open() ){
            if( $comments_num == 0 ){
                $comments = esc_html__('No Comments','listingo');
            } elseif ( $comments_num > 1 ){
                $comments= $comments_num . esc_html__(' Comments','listingo');
            } else {
                $comments = esc_html__( '1 Comment','listingo' );
            }
            $comments = '<a href="' . esc_url( get_comments_link() ) . '">'. $comments .'</a>';
        } else {
            $comments = esc_html__( 'Comments are closed', 'listingo' );
        }
...
function listingo_iframe_match(){   
    $audio_content = listingo_embedded_media( array('audio', 'iframe') );
    $iframe_match = preg_match("/\iframe\b/i",$audio_content, $match);
    return $iframe_match;
}

//Post embedded media
function listingo_embedded_media( $type = array() ){
...
function listingo_link_pages(){
    ...
    'link_before' => '<span>',
    'link_after'  => '</span>',
    'pagelink'    => '<span class="screen-reader-text">' . esc_html__( 'Page', 'listingo' ) . ' </span>%',
    'separator'   => '<span class="screen-reader-text">, </span>',
    ) );
}

// Blog social sharing buttons
function listingo_social_sharing_buttons( $ulClass = '', $tagLine = '', $show_label = false ) {
...
if( ! function_exists('listingo_blog_single_post_navigation') ) {
	function listingo_blog_single_post_navigation() {
				...
				<div class="row">
					<div class="col-lg-6 col-md-6 col-12 nav-left flex-row d-flex justify-content-start align-items-center">
						...
						if( get_next_post_link() ){
							...
							if( has_post_thumbnail() ){
								...
								<div class="thumb">
									<a href="<?php the_permalink(  absint( $nextPost->ID )  ) ?>">
										<?php echo get_the_post_thumbnail( absint( $nextPost->ID ), 'listingo_np_thumb', array( 'class' => 'img-fluid' ) ) ?>
									</a>
								</div>
								<?php
							} ?>
							...
							<div class="detials">
								<p><?php echo esc_html__( 'Prev Post', 'listingo' ); ?></p>
								<a href="<?php the_permalink(  absint( $nextPost->ID )  ) ?>">
					...
					<div class="col-lg-6 col-md-6 col-12 nav-right flex-row d-flex justify-content-end align-items-center">
						...
						if( get_previous_post_link() ){
							...
							?>
							<div class="detials">
								<p><?php echo esc_html__( 'Next Post', 'listingo' ); ?></p>
							...
							<div class="thumb">
								<a href="<?php the_permalink(  absint( $prevPost->ID )  ) ?>">
									<?php echo get_the_post_thumbnail( absint( $prevPost->ID ), 'listingo_np_thumb', array( 'class' => 'img-fluid' ) ) ?>
								</a>
							</div>
							<?php
						} ?>
...
// Listingo Comment Template Callback
function listingo_comment_callback( $comment, $args, $depth ) {
				...
				<div class="desc">
					...
					<div class="d-flex justify-content-between">
						<div class="d-flex align-items-center">
							<h5 class="comment_author"><?php printf( __( '<span class="comment-author-name">%s</span> ', 'listingo' ), get_comment_author_link() ); ?></h5>
							<p class="date"><?php printf( __('%1$s at %2$s', 'listingo'), get_comment_date(),  get_comment_time() ); ?><?php edit_comment_link( esc_html__( '(Edit)', 'listingo' ), '  ', '' ); ?> </p>
							<?php if ( $comment->comment_approved == '0' ) : ?>
								<em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'listingo' ); ?></em>
								<br>
							<?php endif; ?>
						</div>
...
// theme logo
function listingo_theme_logo( $class = '' ) {

    $siteUrl = home_url('/');
    // site logo
		
	$custom_logo_id = get_theme_mod( 'custom_logo' );
	$imageUrl = wp_get_attachment_image_src( $custom_logo_id , 'listingo_site_logo_155x40' );
	
	if( !empty( $imageUrl[0] ) ){
		$siteLogo = '<a class="'.esc_attr( $class ).'" href="'.esc_url( $siteUrl ).'"><img src="'.esc_url( $imageUrl[0] ).'" alt="'.esc_attr( listingo_image_alt( $imageUrl[0] ) ).'"></a>';
	}else{
		$siteLogo = '<a class="'.esc_attr( $class ).'" href="'.esc_url( $siteUrl ).'"><img src="'.esc_url( LISTINGO_DIR_IMGS_URI . 'logo.png' ).'" alt="'.esc_attr( get_bloginfo('name') ).'"></a>';
	}
...
function listingo_pull_right( $id = '', $condation ){
...
// image alt tag
function listingo_image_alt( $url = '' ){
...
function listingo_get_textareahtml_output( $content ) {
...
function listingo_get_slider_shortcode_options( $field ) {
...
// html Style tag
function listingo_inline_bg_img( $bgUrl ){
    $bg = '';
...
function listingo_sidebar_opt(){

    $sidebarOpt = listingo_opt( 'listingo-blog-sidebar-settings' );
    $sidebar = '1';
...
function listingo_blog_pagination(){
	echo '<nav class="blog-pagination justify-content-center d-flex">';
        the_posts_pagination(
            array(
                'mid_size'  => 2,
                'prev_text' => __( '<span class="ti-angle-left"></span>', 'listingo' ),
                'next_text' => __( '<span class="ti-angle-right"></span>', 'listingo' ),
                'screen_reader_text' => ' '
            )
        );
	echo '</nav>';
...
// Themify Icon
function listingo_themify_icon(){
    return [
        'seo_1' 	=> __('Search Icon', 'listingo'),
        'seo_2' 	=> __('Mike Icon', 'listingo'),
        'seo_3' 	=> __('Share Icon', 'listingo'),
    ];
}

// CTA section
function listingo_get_cta_area() {
	$cta_section_title      = !empty( listingo_opt( 'listingo_cta_section_title') ) ? listingo_opt( 'listingo_cta_section_title') : '';
	$cta_section_text       = !empty( listingo_opt( 'listingo_cta_section_text') ) ? listingo_opt( 'listingo_cta_section_text') : '';
	$cta_section_btn_label  = !empty( listingo_opt( 'listingo_cta_section_btn_label') ) ? listingo_opt( 'listingo_cta_section_btn_label') : '';
	$cta_section_btn_url    = !empty( listingo_opt( 'listingo_cta_section_btn_url') ) ? listingo_opt( 'listingo_cta_section_btn_url') : '';
	?>
    <!-- cta-area-start -->
...
function listingo_get_contact_details_area() {
	?>
    <!-- location-area-start -->
    <div class="addres-area black-bg section-padding">
        <div class="container">
            <div class="row">
                <?php
				// Contact items
				$contact_items = listingo_opt('listingo_contact_details_section_fields');
                if ( $contact_items == true ) {
                    ...
                    if( is_array( $contact_items ) && count( $contact_items ) > 0 ){
                        foreach ($contact_items as $value) {
                            switch ($counter) {
                                case $counter == 2:
                                    $icon_img = LISTINGO_DIR_ICON_IMG_URI . 'clock.png';
                                    break;
                                case $counter == 3:
                                    $icon_img = LISTINGO_DIR_ICON_IMG_URI . 'envelope.png';
                                    break;
                                default:
                                    $icon_img = LISTINGO_DIR_ICON_IMG_URI . 'globe.png';
                                    break;
                            }
                            $counter++;
