...
// Final Class
final class Listingo {

	// Theme Version
	private $listingo_version = '1.0';
	...
	private function setup() {
		...
		$enqueu          = new listingo_Enqueue();
		$enqueu->scripts = $this->enqueue();
		$enqueu->listingo_scripts_enqueue_init();
	...
	public function support() {
		// content width
		$GLOBALS['content_width'] = apply_filters( 'listingo_content_width', 751 );


		// text domain for translation.
		load_theme_textdomain( 'listingo', LISTINGO_DIR_PATH . '/languages' );
		...
		// Site logo size
		add_image_size( 'listingo_site_logo_155x40', 155, 40, true );

		// Listing thumb sizes
		add_image_size( 'listingo_listing_thumb_362x250', 362, 250, true );
		
		// Latest post thumbnail size
		add_image_size( 'listingo_widget_post_thumb', 80, 80, true );

		// Single blog post image size
		add_image_size( 'listingo_single_blog_750x375', 750, 375, true );
		add_image_size( 'listingo_np_thumb', 60, 60, true );
		...
		register_nav_menus( array(
			'primary-menu' => esc_html__( 'Primary Menu', 'listingo' ),
			'services' => esc_html__( 'Services', 'listingo' ),
			'useful-links' => esc_html__( 'Useful Links', 'listingo' ),
		) );
	...
	private function enqueue() {
		...
		$cssPath = LISTINGO_DIR_CSS_URI;
		$jsPath  = LISTINGO_DIR_JS_URI;
		...
		$scripts = array(
			'style'   => array(
				...
				array(
					...
					'dependency' => array(),
					'version'    => $this->listingo_version,
				),
				array(
					'handler' => 'listingo-style',
	...
	private function google_font() {
		...
		if ( 'off' !== _x( 'on', 'Google font: on or off', 'listingo' ) ) {
			
			$font_families = array(
				'Raleway:200,200i,300,300i,400,400i,500,600,600i,700|Shadows+Into+Light',
			);
	...
	private function customizer_init(){
		...
		// Instantiate listingo theme customizer
		$listingo_theme_customizer = new listingo_theme_customizer();
	}
	...
	public function epsilon_quickie(){
			...
			'links' => array(
				array(
					'link_to'   => 'listingo_options_panel',
	...
	public function set_elementor_flag(){
		update_option( 'listingo_had_elementor', 'no' );
	}
	// Elementor dsiable default style
	public function elementor_desiable_default_style(){

		$nonce = $_POST['nonce'];
		if ( ! wp_verify_nonce( $nonce, 'listingo-elementor-notice-nonce' ) ) {
			return;
		}
		$reply = $_POST['reply'];
		if ( ! empty( $reply ) ) {
			...
			update_option( 'listingo_had_elementor', 'yes' );
		}
		die();
	...
	public function enqueue_elementor_theme_default_style(){
		...
		if ( $disabled_color_schemes === 'yes' && $disabled_typography_schemes === 'yes' ) {
			wp_enqueue_style( 'listingo-elementor-default-style',  LISTINGO_DIR_CSS_URI. 'elementor-default-element-style.css', array(), $this->listingo_version );
		}
	}
	// Enqueue elementor notice scripts
	public function enqueue_elementor_notice_script(){

		$had_elementor = get_option( 'listingo_had_elementor' );

		if( $had_elementor == 'no' && self::check_elementor_preview_page() ){
			wp_enqueue_script( 'listingo-elementor-notice', LISTINGO_DIR_JS_URI.'listingo-elementor-notice.js', array('jquery'), '1.0', true );
			wp_localize_script(
				'listingo-elementor-notice',
				'listingoElementorNotice',
				array(
					'ajaxurl' => admin_url( 'admin-ajax.php' ),
					'nonce'   => wp_create_nonce( 'listingo-elementor-notice-nonce' ),
				)
			);
		}
