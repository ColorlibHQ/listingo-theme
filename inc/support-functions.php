...
function listingo_post_cats() {
	...
	if ( $cats ) {
		...
		$categories .= '<ul class="nav"><li><span><i class="fa fm fa-th-list"></i>' . esc_html( 'Catagory :', 'listingo' ) . '</span></li>';
...
// Blog categories
function listingo_featured_post_cat(){
...
// social media
if ( ! function_exists( 'listingo_social' ) ) {
	function listingo_social( $args = array() ) {
		...
		$url = listingo_opt( 'listingo_social_url' );
...
// header cart count
function listingo_cart_count( $class = '' ) {

	?>
	<div class="header-wrapicon2 <?php echo esc_attr( $class ); ?>">
		<img src="<?php echo esc_url( LISTINGO_DIR_ASSETS_URI ); ?>img/icons/icon-header-02.png" class="header-icon1 js-show-header-dropdown" alt="<?php esc_attr_e( 'ICON', 'listingo' ); ?>">
...
function listingo_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'listingo_contact7_form_content', 10, 2 );